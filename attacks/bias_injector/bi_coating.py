...
from utils import read_memory, write_memory, get_enabled_coils
...
coil_memory_mapping = {
    0: 2048, 1: 2050, 2: 2052, 3: 2054, 4: 2056,
    5: 2058, 6: 2060, 7: 2062, 8: 2064, 9: 2066,
    10: 2068, 11: 2070, 12: 2072, 13: 2074, 14: 2076, 15: 2078,
}

max_coils = 300

# --- LOGGING SETUP ---
LOG_FILE = "log_bi_coating.csv"
...
# Set up duration
if args.duration > 0:
    end_time_epoch = start_time + args.duration
    infinite = False
else:
    infinite = True
    end_time_epoch = float('inf')

print(f"Attack running... Press Ctrl+C to stop manually.")

# --- Outer state ---
active_coils = []
max_active_coils_during_attack = 0
...
try:
    while time.time() < end_time_epoch:
        ...
        if now - last_scan_time >= args.scan_interval:
            ...
            # Track maximum active coils count
            if len(active_coils) > max_active_coils_during_attack:
                max_active_coils_during_attack = len(active_coils)
