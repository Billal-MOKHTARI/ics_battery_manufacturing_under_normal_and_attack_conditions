...
from MF.modbus_grammar import (
    MODBUS_GRAMMAR, 
    MODBUS_EXCEPTION_CODES,
    get_supported_function_codes,
    get_function_grammar
)
from MF.modbus_packet import ModbusPacket, ModbusResponse
...
class ModbusFuzzer:
    ...
    def __init__(self, 
        ...
        self.fuzzing_start_time: Optional[float] = None
        self.max_duration: Optional[float] = None
        
    def _should_stop_fuzzing(self) -> bool:
        """Check if fuzzing should stop due to duration limit"""
        if self.max_duration is None or self.fuzzing_start_time is None:
            return False
    ...
    def fuzz_all_function_codes(self, 
                               ...
                               function_codes: List[int] = None,
                               max_duration: float = None) -> FuzzingSession:
        """
        ...
        if max_duration:
            self.logger.info(f"Duration limit set to {max_duration} seconds")
        
        for function_code in function_codes:
            # Check timeout before each function code
            if self._should_stop_fuzzing():
                self.logger.info(f"Duration limit reached, stopping fuzzing")
                break
    ...
    def fuzz_function_code(self, 
        ...
        for strategy in strategies:
            # Check timeout before starting new strategy
            if self._should_stop_fuzzing():
                self.logger.debug(f"Stopping fuzzing due to duration limit")
                break
