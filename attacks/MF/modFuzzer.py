...
try:
    from MF.fuzzer_core import ModbusFuzzer, FuzzingStrategy
    from MF.config_manager import ConfigManager, setup_logging
    from MF.modbus_connection import ModbusConnection
    from MF.modbus_packet import ModbusPacket
    from MF.modbus_grammar import get_supported_function_codes
...
def append_csv_log(csv_path: str, session_data: dict) -> None:
    """
    Append a fuzzing session record to a CSV file.
    If the file does not exist, it is created with headers.
    
    session_data expected keys:
        - id (auto-generated if not present)
        - timestamp
        - target
        - duration_sec
        - total_tests
        ...
        - function_codes (comma-separated or 'all')
        - json_report_path
    """
    # Determine next ID
    next_id = 1
    ...
    headers = [
        'id', 'timestamp', 'target', 'duration_sec', 'total_tests',
        'success_rate', 'interesting_findings', 'strategies',
        'function_codes', 'json_report_path'
    ]
...
def grammar_based_fuzzing(dest_ip: str, strategies: List[str] = None,
                          function_codes: List[int] = None, max_duration: float = None,
                          csv_log_file: str = None, output_dir: str = None) -> None:
    ...
    if output_dir:
        print(f"JSON reports will be saved to: {output_dir}")
    
    # Create fuzzer instance
    fuzzer = ModbusFuzzer(dest_ip, dest_port)
    
    try:
        # Run fuzzing session with optional duration limit
        ...
        report_filename = f"fuzzing_session_{timestamp}.json"
        report_path = os.path.join(report_dir, report_filename)
        fuzzer.save_session_report(report_path)
        print(f"\nDetailed report saved to: {report_path}")

        # CSV logging if requested
        if csv_log_file:
            # Store absolute path for clarity (or relative, but absolute is safer)
            abs_report_path = os.path.abspath(report_path)
            csv_data = {
                'timestamp': datetime.now().isoformat(),
                'target': f"{dest_ip}:{dest_port}",
                'duration_sec': f"{summary['duration']:.2f}",
                'total_tests': summary['total_tests'],
                'success_rate': f"{summary['success_rate']:.4f}",
                'interesting_findings': summary['interesting_findings'],
                'strategies': ','.join(strategies),
                'function_codes': ','.join(map(str, function_codes)) if function_codes else 'all',
                'json_report_path': abs_report_path
            }
            append_csv_log(csv_log_file, csv_data)
            print(f"CSV log updated: {csv_log_file}")
...
Examples:
  ...
  # Run fuzzing for 60 seconds only
  python3 modFuzzer.py --grammar 192.168.1.100 --duration 60
  
  # Log session to CSV file and save JSON reports to a directory
  python3 modFuzzer.py --grammar 192.168.1.100 --log_file sessions.csv --output_dir ./reports
...
"""
    ...
    parser.add_argument('--duration', '-d', dest='duration', type=float,
                       help='Maximum duration in seconds for fuzzing (auto-stop)')
    parser.add_argument('--log_file', dest='log_file',
                       help='CSV log file path to record fuzzing sessions')
    parser.add_argument('--output_dir', '-o', dest='output_dir',
                       help='Directory to store JSON session report files (default: current directory)')
    ...
    try:
        ...
        if args.target_ip:
            ...
            grammar_based_fuzzing(args.target_ip, strategies, function_codes,
                                  args.duration, args.log_file, args.output_dir)
